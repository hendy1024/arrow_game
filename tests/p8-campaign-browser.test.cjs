...
test('P8 浏览器总览入口、奖励详情、空白关闭、选关和单页边界', {timeout:45000},async()=>{
 ...
 for(let i=0;i<3;i++){for(const type of ['mousePressed','mouseReleased'])await c.send('Input.dispatchMouseEvent',{type,x:target[0],y:target[1],button:'left',clickCount:1},s);await delay(650);}
 ...
 const stock=await e('__arrowDebug.app.inventory.life');await click('life-rescue-use');assert.equal(await e('__arrowDebug.app.inventory.life'),stock-1);assert.equal(await e('__arrowDebug.app.session.lives'),3);assert.equal(await e('__arrowDebug.app.modal'),null);
