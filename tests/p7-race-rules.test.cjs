...
test('P7 同周期同一关种子一致，日周分别固定，三关指数难度增长', () => {
    ...
    assert.equal(seeds.size, 3); assert.equal(difficulty(1).size, 26); assert.equal(difficulty(3).size, 32);
});
...
test('P7 每日三关比每周同关更轻松，日周各自固定并采用对应保底棋盘',()=>{
 ...
 assert.deepEqual([1,2,3].map(n=>difficulty(n,'weekly').size),[26,29,32]);
 for(let n=1;n<=3;n++){
...
test('P7 难度配置校验、参数生效和成绩版本隔离',()=>{
 ...
 assert.equal(sandbox.module.exports.difficulty(1,'daily').size,25);assert.equal(sandbox.module.exports.difficulty(1,'daily').obstacles,2);assert.equal(sandbox.module.exports.difficulty(1,'weekly').size,26);
