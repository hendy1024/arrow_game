...
test('P3 预算耗尽使用同难度、已验证的保底布局', () => { for (const n of [2, 4, 9, 16, 21]) {
    const r = generate(n, 17, { maxAttempts: 0 });
    assert.equal(r.fallback, true);
    assert.ok(r.validation.valid);
    assert.ok(acceptable(r.validation.metrics, CONFIG.profiles[profileIndex(n)]));
    assert.equal(r.level.lifeLimit, n >= 21 ? 3 : null);
} });
...
test('P3 每档200种子，全量至少1000关经独立求解及实际会话清空', { timeout: 120000 }, () => { const report = require('../scripts/validate-levels.cjs').validateBatch(200); assert.deepEqual(report.failures, []); assert.equal(report.total, 1000); assert.equal(report.tiers.length, 5); });
