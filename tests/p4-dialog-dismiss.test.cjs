...
test('P4 外部空白关闭设置、排行榜和竞速选择，内部空白不关闭，重置按取消处理',()=>{
 ...
 app.action('reset-progress-ask');render();tap(p,1,1);assert.equal(app.currentLevel,15);assert.equal(app.unlocked,15);
...
test('P4 关闭生成及重排弹窗，异步晚到结果不重开页面、不扣库存',async()=>{
 ...
 await app.start(25);app.action('life-accept');const original=app.session;app.generate=()=>new Promise(r=>resolve=r);const task=app.action('item-shuffle');assert.equal(app.modal,'shuffling');app.action('dismiss-modal');app.generate=async(n,s)=>generate(n,s);resolve(generate(25,51));await task;assert.equal(app.session,original);assert.equal(app.session.state,'playing');assert.equal(app.inventory.shuffle,10);assert.equal(app.modal,null);
});
