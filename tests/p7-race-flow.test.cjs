...
function setup(){let time=Date.parse('2026-09-12T01:00:00Z');const data=new Map();const app=new Controller({now:()=>time,seed:()=>1,feedback(){},storage:{get:k=>data.get(k),set:(k,v)=>data.set(k,v)}});app.unlocked=15;app.raceUnlockSeen=true;app.lifeIntroDone=true;app.raceGenerate=async()=>JSON.parse(JSON.stringify(fallbacks));return {app,advance:ms=>time+=ms,data};}
...
test('P7 每日每周赛道可解、确定生成、十关密度和依赖符合验收，兜底亦符合',()=>{
 ...
 const a=generateRound(key,n),b=generateRound(key,n);assert.deepEqual(a,b);const v=solve(a);assert.ok(v.valid);assert.ok(acceptable(v.metrics,profile(n)));assert.equal(a.lifeLimit,3);assert.equal(a.timeLimitMs,null);assert.ok(acceptable(solve(generateRound(key,n,{maxAttempts:0})).metrics,profile(n)));
 }
...
test('P7 解锁、十关实际移动通关记录、暂停计时、无道具、进度隔离',async()=>{
 const {app,advance}=setup();app.unlocked=14;app.action('race');assert.equal(app.modal,'race-locked');app.action('race-close');app.unlocked=15;
 ...
 for(let n=1;n<=10;n++){assert.equal(app.currentLevel,n);assert.equal(app.session.remainingMs,null);assert.equal(app.session.lives,3);advance(10000);clear(app);assert.equal(app.modal,n===10?'race-finished':'won');if(n<10)app.action('next');}
...
test('P7 点错三次结束整轮，重试回第一关，未完成不入历史',async()=>{
 ...
 for(let i=0;i<3;i++){app.clickArrow(id);app.tick(250);}assert.equal(app.modal,'failed');assert.equal(history.read(app.platform.storage).length,0);await app.action('restart');app.action('race-accept');assert.equal(app.currentLevel,1);assert.equal(app.session.lives,3);
});
...
test('P7 第14关通关仅弹一次竞速解锁提示，确认回到通关流程',()=>{
 const {app}=setup();app.unlocked=14;app.raceUnlockSeen=false;app.screen='game';app.currentLevel=14;app.session=new (require('../src/domain/session').Session)({...require('../src/fixtures').fixtures.boundary,number:14});app.clickArrow('a');app.tick(10000);assert.equal(app.unlocked,15);assert.equal(app.modal,'race-unlocked');app.action('race-notice-close');assert.equal(app.modal,'won');assert.equal(app.raceUnlockSeen,true);app.syncModal();assert.equal(app.modal,'won');
});
test('P7 竞速切后台不提前完成动画或提交成绩，返回后继续',async()=>{
 const p=require('./helpers.cjs').fakePlatform();let time=100000;p.now=()=>time;const {app}=require('../src/runtime').mount(p);app.unlocked=15;app.raceGenerate=async()=>JSON.parse(JSON.stringify(fallbacks));await start(app);const id=solve(app.session.level).sequence[0];app.clickArrow(id);assert.ok(app.session.moves.has(id));p.callbacks.hide();assert.ok(app.session.moves.has(id));assert.equal(app.session.removed.size,0);time+=10000;p.callbacks.show();assert.equal(require('../src/race/controller').elapsed(app),10000);app.tick(10000);assert.ok(app.session.removed.has(id));
});
test('P7 好友榜停留跨零点自动刷新，个人历史可重试同步当前最佳',async()=>{
 ...
 for(const [id,elapsed]of [['one',9000],['two',3000]])history.save(app.platform.storage,{id,kind:'daily',period:key,elapsed,finishedAt:app.platform.now()});await app.action('race-sync');assert.equal(sent[0].elapsed,3000);
});
