...
test('P7 第5关通关仅弹一次竞速解锁提示，确认回到通关流程',()=>{
 const {app}=setup();app.unlocked=5;app.raceUnlockSeen=false;app.screen='game';app.currentLevel=5;app.session=new (require('../src/domain/session').Session)({...require('../src/fixtures').fixtures.boundary,number:5});app.clickArrow('a');app.tick(10000);assert.equal(app.unlocked,6);assert.equal(app.modal,'race-unlocked');app.action('race-notice-close');assert.equal(app.modal,'won');assert.equal(app.raceUnlockSeen,true);app.syncModal();assert.equal(app.modal,'won');
});
test('P7 竞速切后台不提前完成动画或提交成绩，返回后继续',async()=>{
 const p=require('./helpers.cjs').fakePlatform();let time=100000;p.now=()=>time;const {app}=require('../src/runtime').mount(p);app.unlocked=6;app.raceGenerate=async()=>JSON.parse(JSON.stringify(fallbacks));await start(app);const id=solve(app.session.level).sequence[0];app.clickArrow(id);assert.ok(app.session.moves.has(id));p.callbacks.hide();assert.ok(app.session.moves.has(id));assert.equal(app.session.removed.size,0);time+=10000;p.callbacks.show();assert.equal(require('../src/race/controller').elapsed(app),10000);app.tick(10000);assert.ok(app.session.removed.has(id));
});
