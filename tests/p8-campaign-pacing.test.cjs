...
test('P8 20关指数曲线，第2关限时高难、第3关障碍、长箭头密集且全部可解',()=>{
 ...
 assert.equal(levels[1].board.width,25);assert.equal(levels[1].board.timeLimitMs,120000);assert.equal(levels[2].board.obstacles.length,2);assert.equal(levels[3].board.obstacles.length,4);assert.equal(levels[19].board.width,32);assert.equal(levels[19].board.timeLimitMs,75000);
