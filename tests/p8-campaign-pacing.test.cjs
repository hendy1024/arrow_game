...
test('P8 20关指数曲线，第2关限时高难、第4关障碍、长箭头密集且全部可解',()=>{
 ...
 assert.equal(p.size,Math.round(20*Math.pow(32/20,t)));assert.equal(p.minDepth,Math.round(32*Math.pow(56/32,t)));
 ...
 assert.equal(b.obstacles.length,p.obstacles);assert.ok(p.obstacles>=previousStones);previousStones=p.obstacles;assert.equal(b.timeLimitMs,p.timeSeconds*1000);assert.ok(p.timeSeconds<=previousTime);previousTime=p.timeSeconds;assert.equal(b.lifeLimit,3);
 }
 assert.equal(levels[1].board.width,20);assert.equal(levels[1].board.timeLimitMs,120000);assert.equal(levels[2].board.obstacles.length,0);assert.equal(levels[3].board.obstacles.length,2);assert.equal(levels[19].board.width,32);assert.equal(levels[19].board.timeLimitMs,75000);
