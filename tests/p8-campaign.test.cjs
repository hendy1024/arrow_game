...
function clear(a){if(a.modal==='life-intro')a.action('life-accept');if(a.modal==='mechanic-intro')a.action('mechanic-accept');a.modal=null;a.session.resume();for(const id of solve(a.session.level).sequence){assert.equal(a.clickArrow(id).type,'allowed');while(a.session.moves.size&&a.session.state==='playing')a.tick(16);}}
test('P8 40关固定配置全部可解，读取独立副本，配置奖励校验',async()=>{
 assert.equal(require('../scripts/check-campaign.cjs').check(),40);
 for(let n=1;n<=40;n++){const a=await catalog.load(n),b=await catalog.load(n);assert.deepEqual(a,b);a.level.arrows.length=0;assert.ok(b.level.arrows.length>0);}
 ...
 await assert.rejects(async()=>catalog.load(41));
});
test('P8 首通奖励与库存一起保存，重复通关、恢复、重置不重复发放',async()=>{
 const a=app();a.unlocked=7;await a.start(7);clear(a);assert.equal(a.inventory.life,12);assert.ok(a.rewardClaims.includes(7));assert.equal(a.unlocked,8);a.events();assert.equal(a.inventory.life,12);
 const data=snapshot(a);assert.ok(validate(data));const b=app();restore(b,data);assert.equal(b.inventory.life,12);assert.ok(b.rewardClaims.includes(7));b.modal=null;b.screen='home';b.platform.isTrial=true;b.action('reset-progress-ask');b.action('reset-progress-confirm');assert.ok(b.rewardClaims.includes(7));
 await b.start(7);clear(b);assert.equal(b.inventory.life,12);
 const old=snapshot(app());old.unlocked=21;delete old.rewardClaims;restore(b,old);assert.equal(b.rewardClaims.length,20);
});
test('P8 总览40关分页，锁定关只看奖励，已过关可重玩',async()=>{
 ...
 a.session=null;await a.start(40);clear(a);a.action('next');assert.equal(a.screen,'map');assert.equal(a.mapPage,1);
});
...
test('P8 关卡总览小屏到手机安全区20个目标均44像素、不重叠、奖励可查看',()=>{
 ...
 a.action('map-level-19');v.render(a,info);assert.ok(!v.buttons.some(b=>b.id==='map-play'));assert.ok(p.ctx.calls.some(c=>c[0]==='fillText'&&String(c[1]).includes('加时')));
 }
});
